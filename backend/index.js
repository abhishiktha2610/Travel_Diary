...
app.post("/add-travel-story", authenticateToken, async (req, res) => {
  ...
  // Assuming visitedLocation is provided as an object with lat and lng
  const { latitude, longitude } = visitedLocation;

  if (typeof latitude !== "number" || typeof longitude !== "number") {
    return res.status(400).json({
      error: true,
      message: "Visited location must contain latitude and longitude",
    });
  }
...
app.put("/edit-story/:id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    travelDiary.imageUrl = imageUrl;
    travelDiary.visitedDate = visitedDate;
...
app.post("/search-nearby-stories", authenticateToken, async (req, res) => {
  ...
  try {
    const nearbyStories = await TravelDiary.find({
      userId: userId,
      visitedLocation: {
        $near: {
    ...
    res.status(200).json({ stories: nearbyStories });
  } catch (error) {
    res.status(500).json({ error: true, message: error.message });
  }
});
